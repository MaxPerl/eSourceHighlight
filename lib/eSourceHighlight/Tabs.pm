...
sub change_tab {
	...
	if ( ref($self->app->current_tab) eq "eSourceHighlight::Tab") {
	...
	$self->app->change_doctype_label();
	$self->app->change_doctype_options();
		
	######################
	# Clear search results
	######################
	$self->app->entry()->search()->clear_search_results();
	
	#######################
	# Clear match braces cursors
	#######################
	foreach my $fcp (@{$self->app->entry->match_braces_fmt}) {
		$fcp->free();
	}
	$self->app->entry->match_braces_fmt([]);
