...
sub add_toolbar {
	...
	$toolbar->item_append("document-properties","Source format",\&set_src_format,$self);
	$toolbar->item_append("view-refresh","Rehighlight",sub {$self->entry->rehighlight_all()},$self);
	my $it_sep2 = $toolbar->item_append(undef,undef,undef,undef);
...
sub about {
	...
	my $popup = pEFL::Elm::Popup->add($self->elm_mainwindow());
	$popup->text_set("<b>eSourceHighlight</b><br/><br/>A simple editor/frontend for the <br/>GNU source-highlight library for not too heavy files written in Perl/pEFL");
